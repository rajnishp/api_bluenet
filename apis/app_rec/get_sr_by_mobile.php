...
$sql = "SELECT sr.id, service,service_type,salary,start_time,end_time, "
    ...
    . "LEFT JOIN `bluenet_v3`.users AS uw ON sr.id = wcm.service_request_id  "
    . "LEFT JOIN `bluenet_v3`.timings AS t  ON sr.id = t.service_request_id "
    . "LEFT JOIN `bluenet_v3`.worker_customer_match AS wcm ON wcm.user_worker_id = uw.id AND wcm.type != 'leave' "
