...
. "; ";*/
$data = date('Y-m', strtotime($_GET['month']));;
$month = explode('-', $data);
...
$sql = "SELECT sr.id, sr.name, sr.user_id as c_user_id, sr.mobile, sr.address, sr.remarks, sr.status, sr.creation, service,service_type,salary,start_time,end_time, r.rating as cem_rating,  "
    ...
    . "WHERE sr.user_cem_id = '".$route[2]."' AND sr.status =  '".$_GET['status']."' AND month( sr.creation ) = '".$month[0]."' AND year( sr.creation ) = '".$month[1]."' ORDER BY `creation` DESC "  ;
